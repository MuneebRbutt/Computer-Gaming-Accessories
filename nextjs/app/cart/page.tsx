          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {items.map(({ product, quantity }) => (
                <div key={product.id} className="flex gap-4 p-4 border border-gray-800 rounded-md bg-card">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-brand font-semibold">Rs {product.price.toLocaleString('en-PK')}</div>
            ...
            <div className="p-4 border border-gray-800 rounded-md bg-card h-max sticky top-24">
              <div className="flex items-center justify-between">
                ...
                <div className="font-semibold">Rs {getTotalPrice().toLocaleString('en-PK')}</div>
              </div>
              <div className="mt-3 grid gap-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <input value={coupon} onChange={(e)=>setCoupon(e.target.value)} className="flex-1 bg-black/40 border border-gray-700 rounded-md px-3 py-2 text-sm" placeholder="Coupon code (SAVE500 / OFF10)" />
                  <Button variant="outline" size="sm" onClick={()=>setApplied(coupon)}>Apply</Button>
                </div>
                {applied && <div className="text-green-400">Applied: {applied.toUpperCase()} (−Rs {discount.toLocaleString('en-PK')})</div>}
              </div>
              ...
              <Link href="/" className="block text-center text-sm text-gray-400 mt-2 hover:text-brand">Continue Shopping</Link>
            </div>
          </div>
        )}
      </main>
